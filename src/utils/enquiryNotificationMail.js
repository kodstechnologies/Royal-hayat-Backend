...
export const enquiryNotificationEmailTemplate = (enquiry) => {
  const enquiryId = escapeHtml(enquiry.enquiryId || "—");
  const submittedAt = formatDateTime(enquiry.createdAt);
  const phone = enquiry.phone != null ? escapeHtml(enquiry.phone) : "N/A";
            ...
            <tr>
              <td style="background: linear-gradient(135deg, #7f1d1d 0%, #991b1b 100%); padding: 24px 32px;">
                ...
                <p style="margin: 10px 0 0; font-size: 14px; color: rgba(255,255,255,0.9);">
                  Submitted from the website Contact Us page
                </p>
              </td>
            </tr>
            <tr>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #fef2f2; border: 1px solid #fecaca; border-radius: 12px;">
                  <tr>
                    <td style="padding: 14px 18px;">
                      <p style="margin: 0 0 4px; font-size: 11px; text-transform: uppercase; color: #991b1b;">Enquiry ID</p>
                      <p style="margin: 0; font-size: 18px; font-weight: 700; color: #7f1d1d; font-family: 'Courier New', Courier, monospace;">${enquiryId}</p>
                    </td>
            ...
            <tr>
              <td style="padding: 16px 32px 24px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                  ${renderRow("Full Name", escapeHtml(enquiry.name))}
                  ${renderRow("Email", escapeHtml(enquiry.email))}
                  ${renderRow("Phone", phone)}
                  ${renderRow("Department", escapeHtml(enquiry.department))}
                  ${renderRow("Message", escapeHtml(enquiry.message).replace(/\n/g, "<br />"))}
                </table>
              </td>
            </tr>
            <tr>
              <td style="padding: 16px 32px 24px; background-color: #f8fafc; border-top: 1px solid #e2e8f0;">
                <p style="margin: 0; font-size: 12px; line-height: 1.6; color: #64748b; text-align: center;">
                  You can reply directly to this email to reach the person who submitted the enquiry.
                </p>
              </td>
            </tr>
          </table>
...
export const sendEnquiryNotificationEmail = async (enquiry) => {
  ...
  if (recipients.length === 0) {
    throw new Error("No enquiry notification recipients configured");
  }
  ...
  await transporter.sendMail({
    ...
    html: enquiryNotificationEmailTemplate(enquiry),
  });
};
