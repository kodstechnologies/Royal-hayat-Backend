...
const sanitizePayload = (body = {}) => {
  ...
  if (body.fullname !== undefined) payload.fullname = String(body.fullname).trim();
  if (body.phone !== undefined) payload.phone = String(body.phone).trim();
  if (body.age !== undefined && body.age !== '') payload.age = Number(body.age);
  if (body.gender !== undefined) payload.gender = String(body.gender).trim();
  if (body.additionalNotes !== undefined) payload.additionalNotes = String(body.additionalNotes).trim();
  if (body.dateOfBirth !== undefined && body.dateOfBirth !== '') {
    const dob = new Date(body.dateOfBirth);
    if (Number.isNaN(dob.getTime())) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid dateOfBirth');
    }
...
const createAppointmentRequest = asyncHandler(async (req, res) => {
  ...
  const created = await AppointmentRequest.create(payload);
...
const getAllAppointmentRequests = asyncHandler(async (req, res) => {
  ...
  const [rows, total] = await Promise.all([
    AppointmentRequest.find({}).sort({ createdAt: -1 }).skip(skip).limit(limit),
...
const getAppointmentRequestById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  if (!OID.test(id)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid appointment request id');
  }
...
const updateAppointmentRequest = asyncHandler(async (req, res) => {
  ...
  const updated = await AppointmentRequest.findByIdAndUpdate(id, payload, {
    new: true,
    runValidators: true,
  });
  if (!updated) {
